...
class Lapiseira{
    ...
    std::vector<std::shared_ptr<Grafite>> bico;
    ...
    void pull(){
        if(grafite != nullptr){
            ...
            fn::write("fail: tambor vazio");
            return;
        }
        grafite = bico.front();
        bico.erase(bico.begin());
    }
    ...
    std::string str() const{
        ...
        if(this->grafite == nullptr){
            ss << "calibre: " << this->calibre_p << ", bico: []";

        }else{
            ss << "calibre: " << this->calibre_p << ", bico: " << this->grafite->str();
        }

        ss << ", tambor: <";
        for(size_t i = 0; i < bico.size(); ++i){
            ss << bico[i] ->str();
        }
        ss << ">";
        return ss.str();
